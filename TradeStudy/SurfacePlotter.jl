...
function CoefThrustFunc(SRPM,SVarPitch)
        ...
        op = simple_op.(Vinf, Omega, Sr, rho; pitch)
        out = solve.(Ref(Srotor), SPVarSections, op)
        T, Q = thrusttorque(Srotor, SPVarSections, out)   #calcs T & Q at each sec w/given conds, sums them for the whole rotor
        SEffPVar, SCTPVar, SCQPVar = nondim(T, Q, Vinf, Omega, rho, Srotor, "propeller")
...
rho = 1.225
SNumofPitches = 20 #Number of pitches to vary
SVarPitch = range(-25, 25, length = SNumofPitches) #Ranging from pitching down to pitching up
SRPM = range(1000, 10000, length = SNumofPitches) #Ranging from 1000 to 10000 RPMs
#! Omega is a Function of RPM Omega = RPM*pi/30  
# I will need Omega = (pi/30).*RPM 
#Omega = 523.5987755982989
#SEffPVar = zeros(SNumofPitches) #Initialize the efficiency array
#SCTPVar = zeros(SNumofPitches) #Initialize the CT array
#SCQPVar = zeros(SNumofPitches) #Initialize the CQ array
